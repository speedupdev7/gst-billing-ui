...
const StockPurchase = () => {
  ...
  const isSameState =
    String(selectedSupplier?.stateCode || "") === PURCHASE_STATE_CODE;
  ...
  const calculateTotals = useCallback(
    (currentItems) => {
      ...
      setTotals({
        ...
        roundOff: (rounded - raw).toFixed(2),
      });
      return updated;
    },
    [isSameState],
  ...
  const removeRow = (id) => {
    if (items.length > 1)
      setItems(calculateTotals(items.filter((i) => i.id !== id)));
  };
  ...
  const normalizeSupplier = (supplier) => ({
    ...
    billingAddress:
      ...
      supplier?.address ||
      [supplier?.address1, supplier?.city, supplier?.pinCode]
        .filter(Boolean)
        .join(", ") ||
      "",
  ...
  const searchSuppliers = async (q) => {
    ...
    try {
      const suppliers =
        supplierCache.length > 0 ? supplierCache : await fetchSuppliers();
      const normalizedQuery = searchText.toLowerCase();
      const filtered = suppliers.filter((supplier) => {
        const normalizedSupplier = normalizeSupplier(supplier);
        return [
          normalizedSupplier.supplierName,
          normalizedSupplier.gstin,
          normalizedSupplier.mobileNo,
          normalizedSupplier.email,
          normalizedSupplier.state,
          normalizedSupplier.billingAddress,
        ].some((value) =>
          String(value || "")
            .toLowerCase()
      ...
      setSupplierSuggestions(filtered.map(normalizeSupplier));
  ...
  const fetchSupplierDetails = async (supplierId) => {
    ...
    try {
      const res = await axios.get(`/api/supplier-master/${supplierId}`);
      return normalizeSupplier(res.data);
    } catch (error) {
      console.error("Unable to fetch supplier details", error);
      return null;
    }
  };
  ...
  const searchItems = async (q) => {
    ...
    try {
      const res = await axios.get(
        `/api/item-master/search?q=${encodeURIComponent(q)}`,
      );
  ...
  const selectSupplier = async (s) => {
    const normalizedSupplier = normalizeSupplier(s);
    setSelectedSupplier(normalizedSupplier);
    setSupplierSearch(normalizedSupplier.supplierName);
    setShowSupplierDropdown(false);

    if (normalizedSupplier.supplierId) {
      const freshSupplier = await fetchSupplierDetails(
        normalizedSupplier.supplierId,
      );
      if (freshSupplier) {
        setSelectedSupplier(freshSupplier);
        setSupplierSearch(freshSupplier.supplierName);
      }
    }
  };
  ...
  const savePurchase = async () => {
    ...
    const validItems = items.filter(
      (item) => item?.itemId && Number(item.qty || 0) > 0,
    );
    ...
    const purchaseData = {
      ...
      stateCode: selectedSupplier?.stateCode || PURCHASE_STATE_CODE,
      placeOfSupply: placeOfSupply || selectedSupplier?.state || "Maharashtra",
      reverseCharge,
      ...
      items: validItems.map((item) => ({
        ...
        discountAmt: Number(item.discA || 0),
  ...
  const handleSaveAndPrint = async () => {
    ...
    if (items.filter((i) => i.itemId).length === 0) {
      setPrintError("Please add at least one item");
      return;
    }
    const payments = buildPaymentsPayload();
    const totalPaid = payments.reduce((s, p) => s + p.amount, 0);
    if (payments.length === 0) {
      setPrintError("Please select a payment method.");
      return;
    }
    if (totalPaid < totals.invoiceTotal - 0.01) {
      setPrintError("Payment is incomplete.");
      return;
    }
    setPrintError(null);
    ...
    try {
      ...
      a.href = url;
      a.download = `invoice_${invoiceNo || Date.now()}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
    } catch (err) {
      setPrintError(err.response?.data?.message || "Failed to save and print.");
    } finally {
    ...
    <div className="min-h-screen p-2 sm:p-4 text-[12px] font-sans text-slate-700 bg-emerald-50/30">
      <div className="max-w-[1500px] mx-auto bg-white rounded-xl overflow-hidden border border-emerald-200 shadow-lg shadow-emerald-900/5">
        ...
        >
          <div className="flex items-center gap-3 pr-4 border-r border-emerald-400/30 mr-2">
            ...
            <div>
              ...
              <p className="text-[9px] text-emerald-200/70 uppercase tracking-widest">
                Purchase Entry
              </p>
            </div>
          </div>
          ...
          <label className="flex items-center gap-2 ml-2 cursor-pointer select-none">
            ...
            >
              <span
                className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${reverseCharge ? "translate-x-4" : ""}`}
              />
            </div>
            ...
            <div className="col-span-12 md:col-span-3 p-4 border-b border-emerald-200/50 bg-teal-50/20">
              ...
              <div className="relative">
                <input
                  ...
                  value={selectedSupplier?.pan || ""}
                  onChange={(e) =>
                    setSelectedSupplier((p) =>
                      p ? { ...p, pan: e.target.value } : null,
                    )
                  }
                />
                <CreditCard
            ...
            <div className="col-span-12 md:col-span-3 p-4 border-r border-b border-emerald-200/50">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setSelectedSupplier((p) =>
                      p ? { ...p, mobileNo: e.target.value } : null,
                    )
                  }
                />
                <Send
            ...
            <div className="col-span-12 md:col-span-4 p-4 border-r border-b border-emerald-200/50">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setSelectedSupplier((p) =>
                      p ? { ...p, billingAddress: e.target.value } : null,
                    )
                  }
                />
                <MapPin
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead
              ...
              style={{
                background: "linear-gradient(180deg, #0f2a1e 0%, #111111 100%)",
              }}
            >
              <tr className="divide-x divide-white/5">
                <th className="p-4 w-14 text-center text-slate-500">Sr.</th>
                <th className="p-4 min-w-[250px] text-emerald-100/90 bg-white/[0.02]">
                  Item Description
                </th>
                ...
                <th className="p-4 w-24 text-center text-teal-200/80 bg-teal-400/[0.04]">
                  MRP
                </th>
                <th className="p-4 w-28 text-center text-emerald-100/90 bg-emerald-400/[0.03]">
                  Rate
                </th>
                <th className="p-4 w-20 text-center text-emerald-100/90 bg-emerald-400/[0.03]">
                  Qty
                </th>
                <th className="p-4 w-20 text-center text-slate-400/60">Free</th>
                <th className="p-4 w-28 text-center opacity-40 font-medium">
                  Gross Amt
                </th>
                <th className="p-4 w-20 text-center text-rose-400/70 bg-rose-400/[0.02]">
                  Disc %
                </th>
                <th className="p-4 w-28 text-center text-slate-200 bg-[#1a1a1a]">
                  Taxable
                </th>
                <th className="p-4 w-20 text-center">GST %</th>
                <th className="p-4 w-32 text-center bg-emerald-500/[0.10] text-emerald-300 font-bold shadow-[inset_0_-2px_0_rgba(16,185,129,0.25)]">
                  Total
                </th>
              ...
              {items.map((item, idx) => (
                <React.Fragment key={item.id}>
                  <tr className="group hover:bg-emerald-50/20 transition-colors">
                    {/* Sr */}
                    <td className="p-3 text-center text-slate-400 font-medium text-[11px] border-r border-slate-50">
                      {idx + 1}
                    </td>
                    ...
                    <td className="p-1 border-r border-slate-50">
                      <input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          if (v === "" || /^\d*\.?\d*$/.test(v))
                            handleItemChange(idx, "discP", v === "" ? 0 : v);
                        }}
                      />
                    </td>
                    ...
                    <td className="p-1 border-r border-emerald-100">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          handleItemChange(idx, "gstP", isNaN(v) ? 0 : v);
                        }}
                      />
                    </td>
                    ...
                    <td className="p-2 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {idx === items.length - 1 && (
                          <button
                            onClick={addNewRow}
                            className="p-1.5 bg-emerald-600 text-white rounded shadow-sm hover:bg-emerald-700 transition-all"
                          >
                            <Plus size={13} />
                          </button>
                        )}
                  ...
                  <tr className="bg-slate-50/40 text-[9px] text-slate-500 border-b border-slate-100">
                    <td
                      colSpan={3}
                      className="py-1 px-4 text-right font-semibold border-r border-slate-200/60"
                    >
                      <span className="text-slate-400">Details:</span>
                    </td>
                    <td
                      colSpan={2}
                      className="py-1 px-2 border-r border-slate-200/60 bg-rose-50/30"
                    >
                    ...
                    <td
                      colSpan={2}
                      className="py-1 px-2 border-r border-slate-200/60"
                    >
                      CGST{" "}
                      <span className="text-slate-900 font-bold ml-1">
              ...
              <div className="bg-white/95 backdrop-blur-sm border border-emerald-200 rounded-xl shadow-2xl shadow-emerald-900/10 overflow-hidden">
                <div className="max-h-[280px] overflow-y-auto">
                  {itemSuggestions.map((s, i) => (
                    <div
                      key={i}
                      className="px-4 py-3 hover:bg-emerald-600 group/item cursor-pointer border-b border-slate-50 last:border-b-0 transition-colors"
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 group-hover/item:bg-emerald-400 group-hover/item:text-white">
                          {s.itemCode}
                        </span>
                      </div>
                      <div className="text-xs text-slate-400 group-hover/item:text-emerald-100 mt-0.5">
                        HSN: {s.hsnCode}
                      </div>
                    </div>
                  ))}
          ...
          <div className="col-span-12 p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200 space-y-4">
                <h4 className="text-[11px] font-black text-emerald-700 uppercase tracking-widest mb-2 border-b border-slate-200 pb-2">
                  Taxes & Adjustments
                </h4>
                <div className="flex gap-4">
                  <div className="flex-1 flex flex-col">
                    ...
                    <span className="font-bold text-black text-base">
                      ₹{" "}
                      {isSameState ? (totals.totalGST / 2).toFixed(2) : "0.00"}
                    </span>
                  </div>
                ...
                <div className="flex gap-4 pt-2 border-t border-slate-200/60">
                  <div className="flex-1 flex flex-col">
                    ...
                    <span className="font-bold text-black text-base">
                      ₹ {!isSameState ? totals.totalGST.toFixed(2) : "0.00"}
                    </span>
                  </div>
                  <div className="flex-1 flex flex-col border-l border-slate-200 pl-4">
              ...
              <div className="bg-emerald-50/30 p-5 rounded-xl border border-emerald-100 space-y-4">
                <h4 className="text-[11px] font-black text-emerald-700 uppercase tracking-widest mb-2 border-b border-emerald-100 pb-2">
                  Purchase Totals
                </h4>
                <div className="flex gap-4">
                  <div className="flex-1 flex flex-col">
              ...
              >
                ...
                <div className="text-right cursor-pointer">
                  <div className="flex items-end justify-end gap-1">
                    <span className="text-2xl font-light text-emerald-200 mb-1.5">
                      ₹
                    </span>
                    <span className="text-5xl font-black tracking-tighter">
                      {totals.invoiceTotal.toLocaleString("en-IN")}
                    </span>
                    <span className="text-2xl font-black mb-1.5 opacity-90">
                      .00
                    </span>
                  </div>
                </div>
          ...
          <div className="col-span-12 md:col-span-3 p-4 border-r border-emerald-200/50">
            ...
            <div className="relative">
              <input
                type="text"
                className="w-full border border-emerald-200 rounded-md p-2 pl-8 bg-white outline-none shadow-sm"
                placeholder="Same as billing or other..."
              />
              <Truck
                size={14}
                className="absolute left-2.5 top-3 text-slate-400"
        ...
        <div className="p-6 bg-slate-50 border-b border-slate-200">
          <span className="text-slate-700 font-bold uppercase text-[11px] block mb-2">
            Narration
          </span>
          <textarea
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, #022c22 0%, #064e3b 50%, #022c22 100%)",
          }}
        >
          {/* Save */}
          {/* <button
          ...
          <button
            onClick={invoiceId ? updateInvoice : savePurchase}
            className="flex items-center gap-2.5 px-6 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-wider transition-all shadow-lg active:scale-95 border-t border-emerald-400/30 text-white"
            style={{
              background: "linear-gradient(180deg, #059669 0%, #047857 100%)",
            }}
          >
            {invoiceId ? (
              <>
                <CheckCircle size={15} strokeWidth={2.5} /> Update Invoice
              </>
            ) : (
              <>
                <Save size={15} strokeWidth={2.5} /> Save Purchase
              </>
            )}
          </button>
          ...
          >
            <Printer
              size={16}
              className={`text-emerald-400 ${isLoadingPrint ? "animate-spin" : ""}`}
            />
          ...
          >
            <Printer
              size={15}
              className={`text-slate-400 ${isLoadingPrint ? "animate-spin" : ""}`}
            />
            {isLoadingPrint ? "Processing..." : "Save & Print"}
          </button>
        ...
        {showPaymentModal && (
          <MultiTransaction
            totals={totals}
            hasInvoiceDiscount={Number(totals.totalDisc || 0) > 0}
          />
        )}
      </div>
    </div>
