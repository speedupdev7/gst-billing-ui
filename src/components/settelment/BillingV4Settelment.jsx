...
const SettlementV1 = () => {
  ...
  const fetchPendingInvoices = async () => {
    try {
      ...
      // Backend already computes correct balance figures on inv.balance —
      // use those directly instead of recalculating on the frontend.
      const bal = inv.balance || {};
      const billAmount = Number(bal.invoiceAmount || 0);
      const paidAmount = Number(bal.paidAmount || 0);
      const balanceAmount = Number(bal.balanceAmount || 0);
  ...
  const saveSettlement = async () => {
    try {
      const paymentData =
        JSON.parse(localStorage.getItem(`payments_${screenKey}`)) || [];
      ...
      const payload = {
        ...
        customerId,
        settlementDate,
        requestedAmount: totals.totalBalance,
        settledAmount: totals.totalSettlement,
        remarks: narration,
        status: "COMPLETED",
        payments: paymentData.map((p) => ({
          ...
          paymentDate: p.paymentDate || settlementDate,
        })),
      };
    ...
    <div className="min-h-screen p-3 md:p-4 text-[12px] font-sans text-slate-700 bg-slate-100 antialiased">
      <div className="max-w-[1600px] mx-auto bg-white rounded-xl overflow-hidden border border-slate-200 shadow-sm flex flex-col">
        ...
        <div className="flex bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 text-white p-4 gap-3 items-center border-b border-white/10 shadow-lg">
          <div className="flex items-center gap-3 pr-4 border-r border-white/20 mr-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-amber-100 uppercase font-medium">
                Adjustment Entry
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 bg-amber-50/30">
          ...
          <div className="p-4 border-r border-b border-amber-200/50">
            ...
            <div className="relative">
              ...
              />
              <DatePicker
                selected={settlementDate ? new Date(settlementDate) : null}
                onChange={(date) =>
                  setSettlementDate(date.toISOString().split("T")[0])
                }
          ...
          <table className="w-full min-w-[900px]">
            <thead className="bg-[#111111] text-slate-300 text-[11px] uppercase tracking-wider sticky top-0 z-10">
              <tr>
                ...
                <th className="p-4 text-center text-emerald-300">
                  Paid Amount
                </th>
                <th className="p-4 text-center text-red-300">
                  Pending Balance
                </th>
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-all">
                  ...
                  <td className="p-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleItemChange(idx, "invoiceNo", e.target.value)
                      }
                    />
                  </td>
                  ...
                  <td className="p-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleItemChange(idx, "invoiceDate", e.target.value)
                      }
                    />
                  </td>
                  ...
                  <td className="p-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleItemChange(idx, "billAmount", e.target.value)
                      }
                    />
                  </td>
                  ...
                  <td className="p-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleItemChange(idx, "paidAmount", e.target.value)
                      }
                    />
                  </td>
                  ...
                  <td className="p-2">
                    <input
                      ...
                      value={Number(item.balanceAmount || 0).toFixed(2)}
                      onChange={(e) =>
                        handleItemChange(idx, "balanceAmount", e.target.value)
                      }
                    />
                  </td>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="block text-[11px] font-black uppercase tracking-[0.2em] text-amber-50">
                    Total Settlement Amount
                  </span>
                  <p className="text-amber-100/60 text-[10px] font-medium uppercase tracking-widest mt-0.5">
                    Click to process payment methods
                  </p>
                </div>
              </div>
              ...
              <div className="text-left md:text-right w-full md:w-auto">
                <div className="flex items-baseline md:justify-end gap-1">
                  <span className="text-xl md:text-2xl font-light text-amber-200">
                    ₹
                  </span>
        ...
        <div className="p-4 md:p-6 bg-slate-50 border-t border-slate-200">
          <label className="text-slate-500 font-bold uppercase mb-2 block text-[10px] tracking-widest">
            Adjustment Remarks
          </label>
          <textarea
        ...
        <div className="flex flex-wrap bg-slate-900 text-white p-4 gap-3 items-center border-t border-slate-800">
          <button
            className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-gradient-to-b from-emerald-500 to-emerald-700 hover:from-emerald-400 hover:to-emerald-600 px-8 py-3.5 rounded-xl font-black text-[11px] uppercase tracking-wider transition-all shadow-lg text-white active:scale-95"
